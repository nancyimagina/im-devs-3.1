            ...
            <div className="overflow-hidden">
              ...
              >
                {TESTIMONIALS.map((item) => (
                  <figure key={item.quote} className="w-full shrink-0 px-2 sm:px-12">
                    <div className="flex h-full flex-col items-center rounded-2xl border border-light-foreground/10 bg-white/50 p-8 text-center sm:p-12">
                      ...
                      <figcaption className="mt-8 border-t border-light-foreground/10 pt-6">
                        <p className="font-display text-lg">{t(item.name)}</p>
                        <p className="mt-1 text-sm text-light-foreground/60">{t(item.role)}</p>
                      </figcaption>
                    </div>
                  </figure>
                ))}
              ...
              <div className="flex gap-2">
                {TESTIMONIALS.map((item, i) => (
                  <button
                    key={item.quote}
                    type="button"
                    aria-label={`${t("Go to testimonial")} ${i + 1}`}
                    onClick={() => setIndex(i)}
                    className={cn(
